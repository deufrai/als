...
2. **Identify the ALS disk image**
   - Look for the disk image whose name starts with `als` and ends with `-arm64.dmg`. For example, it might be `als-v0.7.1-arm64.dmg`.

3. **Mount the disk image**
   - Double-click on the `als-v0.7.1-arm64.dmg` disk image. This will mount the disk image.
